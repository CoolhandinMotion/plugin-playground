import { resolve, normalize, relative, basename, dirname, sep, isAbsolute } from "node:path";
...
/**
 * @param {string} from - absolute base path
...
export async function validateSkills(root, pluginDir, plugin) {
  ...
  if (skillDirs.length === 0) {
    diagnostics.push(
      diagnostic(
        toPosixRelative(root, pluginDir) + "/plugin.json",
        "PLUGIN_SHAPE",
  ...
  for (const skillsEntry of skillDirs) {
    ...
    // Reject escape paths
    const relSkillsDirToRoot = relative(rootNorm, resolvedSkillsDir);
    const isOutsideRoot =
      relSkillsDirToRoot === ".." ||
      relSkillsDirToRoot.startsWith(".." + sep) ||
      isAbsolute(relSkillsDirToRoot);
    if (isOutsideRoot) {
      diagnostics.push(
        diagnostic(
          toPosixRelative(root, pluginDir) + "/plugin.json",
    ...
    for (const relToSkillsDir of skillFiles) {
      ...
      const relFile = toPosixRelative(root, absFile);
      checkedFiles++;
