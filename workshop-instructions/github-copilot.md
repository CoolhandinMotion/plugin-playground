# Workshop Guide — GitHub Copilot CLI

Welcome! This guide walks you through the whole workshop using the **GitHub
Copilot CLI** as your agent. Every step is copy-paste ready. If a step fails,
check section 12 before asking for help — your problem is probably listed
there.

> Guides also exist for Claude Code and Codex (ChatGPT app). They all follow
...
- **GitHub Copilot CLI** installed (`copilot --version` works).
- **Logged in**: run `copilot`, then type `/login` and follow the prompts.
  Installing and browsing plugins works without login, but *invoking* a skill
  talks to the model and requires it — do this now, not mid-exercise.
...
You should see one plugin listed: `plugin-playground`.
...
Expected: `Plugin "plugin-playground" installed successfully. Installed 1 skill.`
...
- `plugin list` shows `plugin-playground@meetup-marketplace (v0.1.0)`.
- `mcp list` shows **no MCP servers**. That's important: the repo bundles an
  MCP *example*, but it's inert — installing the plugin started no process
  and reached no network. You just verified a safety claim yourself instead
  of taking the README's word for it.

Then check the skill was discovered:

```bash
copilot
```

Inside the interactive session, run `/skills list` — you should see
`example-skill` with its description.

## 6. Invoke the skill

Still inside the interactive session (or with `copilot -p "..."`), ask:

> Run the example-skill from the plugin-playground plugin.
...
Don't hand-write it — ask Copilot to draft it for you. Example prompt:
...
error back to Copilot and ask it to fix it. That loop — describe intent,
...
you'll run `copilot` from). Never paste the token itself into any file.
...
`plugins/plugin-playground/.mcp.json` with exactly this content (URL from the
facilitator):
...
After the workshop, if you promoted the Cauldron config (works the same on
every OS — use your editor and file manager, no special commands needed):

1. Delete the file `plugins/plugin-playground/.mcp.json`.
2. Undo the section 10 edit in `plugins/plugin-playground/plugin.json`
   (remove the `"mcpServers": ".mcp.json"` line).
...
| Symptom | Fix |
|---|---|
| `Error: No authentication information found` when invoking a skill | Not a plugin problem. Run `copilot`, then `/login`, then retry the same command. No restart needed. |
| Marketplace registration fails | Run the command from the repository root (the folder with `README.md`). |
| Skill list looks stale after an edit | Installs are cached copies — see section 8. |
